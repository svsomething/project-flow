...
- **Per-card model selection via natural language (#28).** `project-monitor.parse_requested_model()` scans the issue body for the whole words `opus`/`sonnet`/`haiku` (case-insensitive, first match in reading order wins) and passes the result through `invoke_claude()` to `claude_runner.run_claude(..., model=...)`, which appends `--model <alias>` to the CLI invocation. No fixed directive syntax — the user writes the request in the ticket ("please use Opus for this one") and it applies for the whole card lifecycle (plan/iterate/implement/done), since every action routes through the same `invoke_claude()` helper. Two aliases in one body is a known limitation (no NLU disambiguation), not a bug.
